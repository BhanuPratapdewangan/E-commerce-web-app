...
const CrudUpdate = () => {
    ...
    const [product, setProduct] = useState([{ProductName:" ", Price:0, Stock:false ,CategoryName:" "}]);
    ...
    const updateProduct = async(values, {setSubmitting}) => {
       try {
        await axios({
            method:"put",
            url:`http://localhost:8080/updateproduct/${params.id}`,
            data: values
        }) 
        .then(() => {
            ...
            >
                {
                    ...
                    <dl>
                        ...
                        <dt>Stock</dt>
                        <dd><Field type="checkbox" className="form-check-input" name="Stock" value={product[0].Stock} onChange={(e) => {setProduct(e.target.value)}} /></dd>
                        <dt>Category Name</dt>
